...
export default function TimestampedTranscript(props) {
    ...
    if (data) {
        ...
        useTranscriptStore.setState({
            availableLanguages: data['transcripts'].map(({ language, languageCode }) => ({
                language,
                languageCode,
            })),
        });
        ...
        const timestampedTranscriptList = data['transcripts'][targetTranscriptIndex ?? 0]['text'];
        const transcript = timestampedTranscriptList.map((transcriptSegment) => {
            return (
                <a
                    ...
                    data-end={transcriptSegment.start + transcriptSegment.duration}
                    role="button"
                    onClick={() => {
                        useYTPlayerStore.getState().player.seekTo(transcriptSegment.start);
                        UpdateMarker();
        ...
        return (
            <div className="w-full">
                <div className={`${inter.className} flex flex-grow flex-col pb-4`}>
                    {transcript}
                </div>
